      ...
      <div v-else>
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          <div class="bg-white dark:bg-slate-900 p-6 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-800">
            ...
            <p class="text-3xl font-bold font-serif mt-1">{{ libraryStore.totalBooks }}</p>
          </div>
          ...
          <div class="bg-white dark:bg-slate-900 p-6 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-800">
            ...
            <p class="text-slate-500 text-sm font-medium">Currently Reading</p>
            <p class="text-3xl font-bold font-serif mt-1">{{ libraryStore.currentlyReading.length }}</p>
          </div>
          ...
          <div class="bg-white dark:bg-slate-900 p-6 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-800">
            ...
            <p class="text-slate-500 text-sm font-medium">Finished</p>
            <p class="text-3xl font-bold font-serif mt-1">{{ libraryStore.finished.length }}</p>
          </div>
          ...
          <div class="bg-white dark:bg-slate-900 p-6 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-800">
            ...
            <p class="text-slate-500 text-sm font-medium">To Read</p>
            <p class="text-3xl font-bold font-serif mt-1">{{ libraryStore.toRead.length }}</p>
          </div>
        </div>

        <section v-if="libraryStore.currentlyReading.length > 0" class="mb-8">
          <div class="flex items-center justify-between mb-6">
            <h2 class="font-serif text-2xl font-bold">Currently Reading</h2>
          </div>
          <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            >
              ...
              <h3 class="text-xl font-bold mt-4 leading-tight">{{ item.book.title }}</h3>
        ...
        <div class="grid grid-cols-1 xl:grid-cols-3 gap-8">
          ...
          <div class="xl:col-span-2 space-y-6">
            <h2 class="font-serif text-2xl font-bold">Recently Added</h2>
            ...
            <div v-if="libraryStore.recentlyAdded.length === 0" class="bg-white dark:bg-slate-900 p-8 rounded-2xl text-center border border-slate-100 dark:border-slate-800">
              ...
              <p class="text-slate-500">Your library is empty. Start by adding a new book!</p>
            </div>
                ...
                <table class="w-full text-left border-collapse min-w-[500px]">
                  <thead>
                    <tr class="border-b border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-800/50">
                      ...
                      <th class="px-6 py-4 text-xs font-bold text-slate-500 uppercase tracking-wider text-right">Added On</th>
                    </tr>
                  </thead>
                  <tbody class="divide-y divide-slate-50 dark:divide-slate-800">
                    ...
                    >
                      ...
                      <td class="px-6 py-4 text-right text-sm text-slate-500 whitespace-nowrap">
          ...
          <div class="space-y-8">
            <section>
              ...
              <div class="bg-white dark:bg-slate-900 p-6 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-800 space-y-4">
                <RouterLink to="/books/add" class="w-full flex items-center gap-4 p-4 rounded-xl border-2 border-primary/10 hover:border-primary hover:bg-primary/5 transition-all group cursor-pointer">
                  <div class="p-2 bg-primary/10 text-primary rounded-lg group-hover:bg-primary group-hover:text-white transition-colors">
                ...
                <RouterLink to="/catalog" class="w-full flex items-center gap-4 p-4 rounded-xl border-2 border-slate-50 dark:border-slate-800 hover:border-primary/20 hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-all group cursor-pointer">
                  <div class="p-2 bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 rounded-lg group-hover:bg-primary/10 group-hover:text-primary transition-colors">
